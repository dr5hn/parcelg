...
class AuthAPIController extends AppBaseController
{
    ...
    private function validateData($requestArr, $required)
    {
        foreach ($required as $field) {
            if (!array_key_exists($field, $requestArr)) {
                return false;
            }
    ...
    {
        if ($bearerToken !== $this->OT_VERIFICATION_TOKEN) {
            return false;
        }
        return true;
    }
    ...
    public function authenticate(Request $request)
    {
        ...
        if (!$goodFellow) {
            return $this->sendError('I don\'t know who you are. Please verify your self.', 401);
        }
        $data = $request->all();
        $fieldsToValidate = ['first_name', 'last_name', 'phone'];
        $validator = $this->validateData((array)$data, $fieldsToValidate);
        if (!$validator) {
            return $this->sendError('Error !! Some Fields are missing/empty, Please check all required fields.', 206);
        }
